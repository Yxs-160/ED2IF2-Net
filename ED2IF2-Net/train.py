...
import utils
from models.ED2IF2 import ED2IF2_Net
...
def test(epoch, model, dataset, config, testlist):
    ...
    with torch.no_grad():
        gridworldcoords = utils.sample_grid_points(config.mcube_znum, config.mcube_znum,
                                                   config.mcube_znum)  # sampled gridpoints under camera's view
        ...
        for testdata in testlist:
            ...
            cam_id = 32
            if (cat_id == '02958343' or cat_id == '04090263' or cat_id == '04530566'):
                cam_id = 6
            # test
...
def train_epoch(epoch, model, optimizer, data_iter, config):
    ...
    for batch_idx, batch in enumerate(data_iter):
        ...
        Baseloss, Deformedloss, SDFloss, Laploss = model(points, values, gradients, mcimage, transmat, scale)
        loss = Baseloss + Deformedloss + SDFloss + Laploss
        ...
        if ((batch_idx + 1) % config.plot_every_batch == 0 or batch_idx == len(data_iter) - 1):
            # plot the loss
            logline = 'epoch:%d//%d, batch:%d//%d, base_loss:%f, deformed_loss:%f, sdf_loss:%f, lap_loss:%f, total_loss:%f' % (epoch+1, config.epochs, batch_idx+1, len(data_iter), Baseloss.item(), Deformedloss.item(), SDFloss.item(), Laploss.item(), loss.item())
            print(logline)
            utils.print_log(config.log, logline)

    # save the trained model
    if ((epoch + 1) % config.save_every_epoch == 0):
        utils.save_checkpoint(epoch, model, optimizer, loss.item(),
                              config.model_dir + '/model' + str(epoch + 1) + '.pt.tar')
...
def train(config):
    ...
    if (config.load_pretrain and os.path.exists(config.model_dir + '/best_model.pt.tar')):
        epoch, model, optimizer, best_test_loss = utils.load_checkpoint(config.model_dir + '/best_model.pt.tar', model,
                                                                        optimizer)
    ...
    '''
    model.eval()
    test(epoch, model, trainset, config, config.testlist)
    exit()
    '''
    # training
    while (epoch < config.epochs):
        ...
        if (best_test_loss > loss):
            ...
            best_test_loss = loss
            utils.save_checkpoint(epoch, model, optimizer, loss.item(), config.model_dir + '/best_model.pt.tar')
